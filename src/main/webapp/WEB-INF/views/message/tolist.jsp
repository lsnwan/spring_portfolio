...
<body style="position: relative; " data-spy="scroll" data-target="navbar-example" data-offset="25">
					...
					<div class="col-md-3">
						<div class="card">
							<div class="card-body">
								...
								<div class="card-text" style="margin-bottom: 5px;">
									<a class="btn btn-secondary btn-block" href="/message/tolist">
										받은 쪽지함
									</a>
								</div>
								<div class="card-text" style="margin-bottom: 5px;">
								...
								<div class="card-text">
									<div class="row">
										<div class="col-md" style="margin-bottom: 10px;">
											...
											<button class="btn btn-primary btn-sm" id="reSend">답장</button>
											<button class="btn btn-light btn-sm" id="readMessage">읽음</button>
											&nbsp;<small>${noRead} / ${total }</small>
										</div>
										<div class="col-md-5 text-right">
											<form action="/message/tolist?type=${type }&keyword=${keyword}" onsubmit="return frCheck();" name="fr" method="get">
													...
													<c:otherwise>
														...
														<tr>
															...
															<td class="messageFromuser">
																<c:choose>
																	<c:when test="${isRead == 1 }">
																		<small style="color: #2478FF; font-size: 12px">
																			${message.fromuser }
																		</small>																
																	</c:when>
																	<c:otherwise>
															...
															<td class="messageContenttd">
																<span class="messageText">
																	<c:choose>
																		<c:when test="${isRead == 1 }">
																			<a href="/message/read?no=${message.no }" style="color: #2478FF;">
																				${message.content }
																			</a>																
																		</c:when>
																		<c:otherwise>
																			<a href="/message/read?no=${message.no }" style="color: gray;">
																				${message.content }
																			</a>																
																		</c:otherwise>
																	</c:choose>
	...
	function page(idx) {
		...
		if(type == "" && keyword == "") {
			location.href = "${path}/message/tolist?&pagenum=" + pagenum;
		} else {
			location.href = "${path}/message/tolist?&pagenum=" + pagenum + "&type=" + type + "&keyword=" + keyword;			
		}
