...
<body style="position: relative; " data-spy="scroll" data-target="navbar-example" data-offset="25">
					...
					<div class="col-md-3">
						<div class="card">
							<div class="card-body">
								...
								<div class="card-text" style="margin-bottom: 5px;">
									<a class="btn btn-secondary btn-block" href="/message/sendlist">
										보낸 쪽지함
									</a>
								</div>
								<div class="card-text" style="margin-bottom: 5px;">
								...
								<div class="card-text">
									<div class="row">
										...
										<div class="col-md-5 text-right">
											<form action="/message/sendlist?type=${type }&keyword=${keyword}" onsubmit="return frCheck();" name="fr" method="get">
												<div class="input-group md-3">
													<div class="input-group-prepend">
														<select class="form-control form-control-sm" name="type" id="type">
															<option value="-" <c:out value="${type eq '-'? 'selected' : '' }"/>>-</option>
															<option value="N" <c:out value="${type eq 'N'? 'selected' : '' }"/>>이름</option>
														...
														<tr>
															...
															<td class="messageContenttd">
																<span class="messageText">
																	<a href="/message/sendRead?no=${message.no }" style="color: gray;">
																		${message.content }
																	</a>																
																</span>
															</td>
															...
															<td class="sendCancel">
																<c:choose>
																	<c:when test="${isSendCancel == 0 }">
																		
																		<c:if test="${isRead == 1 && isToAvail == 1 }">
																			<span onclick="sendCancel(${message.no}, '${message.touser }');" style="cursor: pointer; color: #2478FF;">발송취소</span>																
																		</c:if>
																		<c:if test="${isRead == 1 && isToAvail == 0 }">
																			취소완료
																		</c:if>
																	...
																	<c:otherwise>
																		...
																		<c:if test="${(isRead == 0 && isToAvail == 1) || (isRead == 0 && isToAvail == 0) }">
																			읽음
																		</c:if>
																		<c:if test="${isRead == 1 && isToAvail == 0 }">
																			읽지않음
	...
	function page(idx) {
		...
		if(type == "" && keyword == "") {
			location.href = "${path}/message/sendlist?&pagenum=" + pagenum;
		} else {
			location.href = "${path}/message/sendlist?&pagenum=" + pagenum + "&type=" + type + "&keyword=" + keyword;			
		}
		
	}
	
	// 발송 취소 버튼
	function sendCancel(no, toUser) {
		
		var token = $("meta[name='_csrf']").attr("content");
		var header = $("meta[name='_csrf_header']").attr("content");
		
		if(confirm("발송 취소 전에 " + toUser + "님이 받은쪽지함에 접속하였다면,\n‘읽지않음’ 상태라도 목록에서 일부 내용을 확인했을 수 있습니다.")) {
			$.ajax({
				type: 'post',
				url: '/message/sendCancel',
				data: {no: no},
